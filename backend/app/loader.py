...
from app.db import get_qdrant, setup_db, add_chunks_to_vector_db
...
async def process_docs(docs_dir=settings.DOCS_DIR):
    ...
    for doc_name, doc_text, metadata in tqdm(docs, desc="Splitting documents"):
        ...
        for chunk_idx, chunk_text in enumerate(doc_chunks):
            chunk = {
                'chunk_id': f'{doc_id}',  # استفاده از UUID کامل
                'text': chunk_text,
                'doc_name': doc_name,
                'vector': None,
                'metadata': metadata
...
async def load_knowledge_base():
    ...
    await setup_db(qdrant)
    chunks = await process_docs()
    if chunks:
        print('\nAdding chunks to vector db')
        await add_chunks_to_vector_db(qdrant, chunks)
