...
class RAGAssistant:
    def __init__(self, chat_id, qdrant, history_size=30, max_tool_calls=3):
    ...
    async def _handle_tool_calls(self, tool_calls, chat_messages):
        ...
        for call in tool_calls[:self.max_tool_calls]:
            ...
            kb_result = await kb_args(self.qdrant)
    ...
    async def _run_step(self, message):
        history = await get_chat_messages(self.qdrant, self.chat_id, last_n=self.history_size)
        ...
        await add_chat_messages(self.qdrant, self.chat_id, [user_db_msg, assistant_db_msg])
