...
from app.assistants.prompts import BASE_PROMPT, MAIN_SYSTEM_PROMPT, RAG_SYSTEM_PROMPT
...
class LocalRAGAssistant:
    def __init__(self, rdb, history_size=15, max_tool_calls=3, log_tool_calls=True, log_tool_results=False):
        ...
        self.rdb = rdb
        self.chat_history = []
        self.main_system_message = {
            'role': 'system',
            'content': f"{BASE_PROMPT.strip()}\n\n{MAIN_SYSTEM_PROMPT.strip()}"
        }
    ...
    async def run(self):
        self.console.print('🤖 منتظر سوالات شما هستم:\n', style='cyan')
        while True:
